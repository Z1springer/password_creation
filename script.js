...
function generatePassword() {
  ...
  var passLen = prompt("How long would you like your password to be? (between 8-128)")
  console.log(userLen + passLen)
  while (passLen < 8 || passLen > 128) {
    alert("You have chosen poorly")
    passLen = prompt("How long would you like your password to be? (between 8-128)")
  }
    ...
    if (capsC === false && lowerCaseC === false && specCharC === false && numbersC === false) {
      alert("Please choose at least one set of characters for your password, or did you believe it would conjure itself from the depths of the web?")
    }

  // Create multiple if statements that checks if the user said yes or no to different confirms that we asked them, and based on their response, !!push those specific characters to our empty array!!
  ...
  // Need a for loop to loop over my final array, it should run based on the results of the prompts, however many characters specified by the user. Choose randomly from our final array that holds all the tags requested by the user and save them to our finalPass var

  for (var i = 0; i < passLen; i++) {
    var randomNum = Math.floor(Math.random() * myFinalArrOfChar.length)//Length of final array 
    finalPass = (finalPass += myFinalArrOfChar[randomNum])
